...
u18_df["merg_id"] = u18_df["year"].astype(str)+u18_df["id"].astype(str)+u18_df["area"].astype(str)
...
def inc_hunn(df):
    try:
        df.loc[ (df[3] != -1 ), "player"] = df["player"].astype(str) + str("分") + df[3].astype(str)
        del df[3]
    except:
        try:
...
#試合情報を結合
df_merged = pd.merge(df_merge,u18_df, on="merg_id").sort_values(["year","area","id","n_time","a_time"],ascending=[False,True,True,True,True]).reset_index(drop=True)
